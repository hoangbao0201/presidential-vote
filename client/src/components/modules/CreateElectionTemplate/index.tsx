    ...
    const handleCreateElection = async () => {
        ...
        try {
            ...
            if (error.code === "NETWORK_ERROR") {
                ...
                toast.error("Có lỗi xảy ra khi tạo cuộc bầu cử. Vui lòng thử lại.");
            }
        }
    };
    ...
    return (
        ...
        <div className="flex justify-center items-center min-h-screen">
            ...
            <div className="p-8 rounded-xl max-w-2xl w-full border border-gray-700 shadow-lg">
                <h2 className="text-2xl text-center font-bold uppercase mb-6">Tạo Cuộc Bầu Cử</h2>

                {isAdmin ? (
                    <div>
                        <div className="mb-4">
                            <label className="block text-sm font-semibold">Tên cuộc bầu cử</label>
                            <input
                                type="text"
                        ...
                        <div className="mb-4">
                            <label className="block text-sm font-semibold">Thời gian cuộc bầu cử (phút)</label>
                            <input
                                type="number"
                                min="1"
                                className="mt-2 p-2 border rounded-md w-full text-black"
                                value={electionData.durationInMinutes}
                                onChange={(e) => setElectionData({ ...electionData, durationInMinutes: +e.target.value })}
                            />
                        </div>

                        <div className="mb-4">
                            <label className="block text-sm font-semibold">Danh sách ứng cử viên</label>
                            <input
                                type="text"
                                className="mt-2 p-2 border rounded-md w-full text-black"
                                ...
                                onKeyDown={(e) => {
                                    if (e.key === "Enter" && (e.target as HTMLInputElement).value) {
                                        setElectionData({
                                            ...electionData,
                                            candidates: [...electionData.candidates, (e.target as HTMLInputElement).value],
                                        });
                                        (e.target as HTMLInputElement).value = ""; // Clear the input after adding
                                    }
                                }}
                            />
                            <div className="mt-2">
                                {electionData.candidates.map((candidate, index) => (
                                    <span key={index} className="linline-block mr-2 bg-gray-400 p-1 rounded text-black overflow-x-auto whitespace-nowrap">
                                        {candidate}
                                    </span>
                                ))}
                            </div>
                        ...
                        <div className="mb-4">
                            <label className="block text-sm font-semibold">URL ảnh ứng cử viên</label>
                            <input
                                type="text"
                                className="mt-2 p-2 border rounded-md w-full text-black"
                                ...
                                onKeyDown={(e) => {
                                    if (e.key === "Enter" && (e.target as HTMLInputElement).value) {
                                        setElectionData({
                                            ...electionData,
                                            imageUrl: [...electionData.imageUrl, (e.target as HTMLInputElement).value],
                                        });
                                        (e.target as HTMLInputElement).value = ""; // Clear the input after adding
                                    }
                                }}
                            />
                            <div className="mt-2">
                                {electionData.imageUrl.map((imageUrl, index) => (
                                    <span key={index} className="block mr-2 bg-gray-400 p-1 rounded text-black overflow-hidden whitespace-nowrap w-full mb-1">
                                        {imageUrl}
                                    </span>
                                ))}
                            </div>
                        ...
                        <div className="mb-4">
                            <label className="block text-sm font-semibold">Danh sách người tham gia bầu cử</label>
                            <input
                                type="text"
                                className="mt-2 p-2 border rounded-md w-full text-black"
                                ...
                                onKeyDown={(e) => {
                                    if (e.key === "Enter" && (e.target as HTMLInputElement).value) {
                                        setElectionData({
                                            ...electionData,
                                            allowedVoters: [...electionData.allowedVoters, (e.target as HTMLInputElement).value],
                                        });
                                        (e.target as HTMLInputElement).value = ""; // Clear the input after adding
                                    }
                                }}
                            />
                            <div className="mt-2">
                                {electionData.allowedVoters.map((allowedVoters, index) => (
                                    <span key={index} className="block mr-2 bg-gray-400 p-1 rounded text-black overflow-hidden whitespace-nowrap w-full mb-1">
                                        {allowedVoters}
                                    </span>
                                ))}
                        ...
                        <div className="mb-4">
                            <label className="block text-sm font-semibold">URL ảnh cuộc bầu cử</label>
                            <input
                                type="text"
                                className="mt-2 p-2 border rounded-md w-full text-black"
                                value={electionData.imageUrlElection}
                                onChange={(e) => setElectionData({ ...electionData, imageUrlElection: e.target.value })}
                            />
                        </div>

                        <div className="mb-4">
                            <label className="block text-sm font-semibold">Mô tả cuộc bầu cử</label>
                            <textarea
                                className="mt-2 p-2 border rounded-md w-full text-black"
                                value={electionData.describe}
                                onChange={(e) => setElectionData({ ...electionData, describe: e.target.value })}
                            />
                        </div>
                    ...
                    <p className="text-red-500">Bạn không có quyền tạo cuộc bầu cử.</p>
                )}
            </div>
